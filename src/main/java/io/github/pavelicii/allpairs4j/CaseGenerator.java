...
final class CaseGenerator {
    ...
    private void rankCandidates(List<Item> prefix, Item[] assigned, int parameter) {
        ...
        final boolean contextual = !this.optionalParameterNames.isEmpty()
                && target.canScoreProjections(assigned, parameter);
    ...
    private long[] candidateWeights(List<Item> prefix, Item[] assigned, Item item, boolean contextual) {
        ...
        if (contextual) {
            ...
            final CompactTable.ProjectionScore projection =
                    this.combinationStorage.getTargetTable().scoreProjections(assigned, item);
    ...
    private List<List<Item>> createItemMatrix(List<Parameter> parameters) {
        CompactTable.checkedCapacities(parameters.stream().mapToInt(parameter ->
                parameter.size() + (needsImplicitAbsence(parameter) ? 1 : 0)).toArray(), this.n);
