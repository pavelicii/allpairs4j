...
class CompactTable {
    ...
    CompactTable(int[] domainSizes, int strength, boolean constrained, int[] ignoredValues) {
        ...
        for (int size = 1; size <= strength; size++) {
            for (int count = size; count <= domainSizes.length; count++) {
                this.binomial[size][count] = Math.addExact(
                        this.binomial[size][count - 1], this.binomial[size - 1][count - 1]);
            }
        }
        ...
        for (int size : domainSizes) {
            ...
            for (int i = Math.min(processed, strength); i > 0; i--) {
                ...
                if (capacity > MAX_CAPACITY) {
                    throw new IllegalArgumentException("Combination capacity exceeds supported integer indexing"
                            + " at strength " + i);
                }
    ...
    void removeImpliedTargets(CompactTable larger) {
        for (Group extension : larger.groups) {
            ...
            for (int omitted = extension.parameters.length - 1; omitted >= 0; omitted--) {
                ...
                for (int index = extension.nextRequiredIndex(0); index >= 0;
                        index = extension.nextRequiredIndex(index + 1)) {
    ...
    int countNewCombinations(List<Item> prefix, Item candidate) {
        if (this.strength == 1) {
            ...
            return group.isCoverageTarget(candidate.getValueIndex()) && !group.isCovered(candidate.getValueIndex())
                    ? 1 : 0;
        }
        if (this.strength == 2) {
    ...
    private int countNewPairs(List<Item> prefix, Item candidate) {
        ...
        for (final Item item : prefix) {
            ...
            final int rank = (int) ((long) left * (2L * this.domainSizes.length - left - 1) / 2)
                    + right - left - 1;
